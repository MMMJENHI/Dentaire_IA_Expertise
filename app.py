...
st.markdown("Isolation anatomique haute précision et diagnostic de densité matricielle.")
...
if raw_img is not None:
    ...
    with col1:
        st.subheader("🔎 Visualisation en Couleur")
        # Conversion en RGB pour la couleur
        img_visu = cv2.cvtColor(img_gray, cv2.COLOR_GRAY2RGB)
        
        # 1. Tracer l'axe du canal supérieur (Gris très discret, presque invisible)
        cv2.line(img_visu, (x_top, y_top_canal), (x_start_tiers, y_start_tiers), (60, 60, 60), 1)
        
        # 2. COLORER LE TIERS APICAL (Cyan pour l'isolation propre)
        # On utilise l'axe rectifié pour éviter l'espace interdentaire
        cv2.line(img_visu, (x_start_tiers, y_start_tiers), (x_apex, y_apex_point), (0, 255, 255), 10)
        
        # 3. COLORER L'APEX (Vert = Sain, Rouge = Patho)
        # Le cercle écrase toute trace noire résiduelle à l'extrémité
        couleur_apex = (0, 255, 0) if h_apex_final >= 0.45 else (255, 0, 0)
        cv2.circle(img_visu, (x_apex, y_apex_point), 22, couleur_apex, -1)
        
        st.image(img_visu, use_container_width=True, caption=f"Analyse localisée sur l'axe X : {x_apex}")

    with col2:
        st.subheader("📈 Profil de Densité Clinique")
        fig = go.Figure()
        fig.add_trace(go.Scatter(y=H_values, mode='lines', line=dict(color='#00fbff', width=5), name="Densité H"))
        fig.add_hline(y=0.45, line_dash="dash", line_color="red", annotation_text="SEUIL")
        fig.add_hrect(y0=0, y1=0.45, fillcolor="red", opacity=0.15)
        fig.update_layout(template="plotly_dark", height=400, yaxis_title="Valeur H", xaxis_title="Profondeur Tiers Apical")
        st.plotly_chart(fig, use_container_width=True)

    st.divider()
    
    if st.button("🚀 LANCER L'EXPERTISE FINALE"):
        with st.spinner('Analyse matricielle...'):
            time.sleep(1)
    ...
    st.write(f"""
    Le système isole les **derniers millimètres** (Tiers Apical en couleur Cyan). 
    Le point de sortie est automatiquement évalué : s'il est **Vert**, la densité est suffisante (>0.45). 
    S'il est **Rouge**, une chute de densité est confirmée.
    """)
else:
